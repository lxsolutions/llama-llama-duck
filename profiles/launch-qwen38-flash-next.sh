...
# NOTE: an earlier revision added --mlock here on a +2% measurement. That
# measurement was taken on the tensor-split path, which is now known to be
# corrupt for this architecture, and --mlock additionally fails outright unless
# the memlock rlimit is raised (default 8 MB cannot lock a multi-GB model).
# Not enabled.
